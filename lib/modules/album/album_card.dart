...
class AlbumCard extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final fetchAllTrack = useCallback(
      () async {
        await ref.read(
          metadataPluginAlbumTracksProvider(album.id).future,
        );
    ...
    final isLoading =
        (isPlaylistPlaying && isFetchingActiveTrack) ||
            updating.value;

    final description =
        '${album.albumType.name} • ${album.artists.asString()}';
    ...
    final onPlaybuttonPressed = useCallback(
      () async {
        ...
        try {
          ...
          if (fetchedTracks.isEmpty || !context.mounted) {
            return;
          }
